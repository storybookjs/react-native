...
describe('stories API', () => {
  ...
  describe('showSettingsPage', () => {
    it('navigates to the specified settings page', () => {
      const navigate = jest.fn();
      const store = createMockStore();
      const {
        api: { showSettingsPage },
        state,
      } = initStories({ store, navigate, viewMode: 'story', storyId: 'a--2' });

      showSettingsPage('about');
      const { viewMode } = store.getState(state);
      expect(viewMode).toEqual('settings');
    });
  });
});
