...
describe('preview.story_store', () => {
  ...
  describe('STORY_INIT', () => {
    const story = () => 0;
    const storyBundle = make('kind', 'story', story);
    ...
    it('supports path params', () => {
      ...
      expect(store.setSelection.mock.calls[0][0].getDecorated()).toEqual(story);
    });
  ...
  describe('splitPath', () => {
    it('should not split with no separator', () => {
      const split = splitPath('a|b.c', { rootSeparator: null, groupSeparator: null });
      expect(split).toEqual({
        root: null,
        groups: ['a|b.c'],
      });
    });
    it('should split with group separator', () => {
      const split = splitPath('a|b.c', { rootSeparator: null, groupSeparator: '.' });
      expect(split).toEqual({
        root: null,
        groups: ['a|b', 'c'],
      });
    });
    it('should split with root and group separators', () => {
      const split = splitPath('a|b.c', { rootSeparator: '|', groupSeparator: '.' });
      expect(split).toEqual({
        root: 'a',
        groups: ['b', 'c'],
      });
    });
    it('should split with default settings', () => {
      const split = splitPath('a|b.c/d', { rootSeparator: '|', groupSeparator: /\/|\./ });
      expect(split).toEqual({
        root: 'a',
        groups: ['b', 'c', 'd'],
      });
