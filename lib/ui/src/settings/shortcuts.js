...
class ShortcutsScreen extends Component {
  ...
  onKeyDown = e => {
    ...
    const error = !!Object.entries(shortcutKeys).find(
      ([feature, { shortcut: existingShortcut }]) =>
        feature !== activeInputField &&
        existingShortcut &&
        shortcutMatchesShortcut(shortcut, existingShortcut)
    );

    return this.setState({
      shortcutKeys: { ...shortcutKeys, [activeInputField]: { shortcut, error } },
    });
  };
  ...
  onBlur = () => {
    ...
    if (shortcutKeys[activeInputField]) {
      const { shortcut, error } = shortcutKeys[activeInputField];
      if (!shortcut || error) {
        return this.restoreDefault();
      }
  ...
  saveShortcut = async () => {
    ...
    await setShortcut(activeInputField, shortcutKeys[activeInputField].shortcut);
    this.setState({ successField: activeInputField });
  };
  ...
  restoreDefault = async () => {
    ...
    const defaultShortcut = await restoreDefaultShortcut(activeInputField);
    this.setState({
      shortcutKeys: {
        ...shortcutKeys,
