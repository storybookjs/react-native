...
const defaultAlphabeticalSort = (a, b) => a.id.localeCompare(b.id);

const calculateTreeState = memoize(50)(
  ({ dataset, selectedId }, { lastSelectedId, unfilteredExpanded, storySort }) => {
    if (selectedId === lastSelectedId) {
      return null;
    }

    let storySortResolved = storySort;
    if (Object.values(dataset).length > 0) {
      const leafKey = Object.keys(dataset).find(key => dataset[key].isLeaf);
      if (
        dataset[leafKey] &&
        dataset[leafKey].parameters &&
        dataset[leafKey].parameters.options &&
        dataset[leafKey].parameters.options.storySort
      ) {
        storySortResolved = dataset[leafKey].parameters.options.storySort;
      }
    }
...
class TreeState extends PureComponent {
  state = {
    ...
    filter: null,
    lastSelectedId: null,
    storySort: defaultAlphabeticalSort,
  };
  ...
  render() {
    const {
      events,
      state: { filter, unfilteredExpanded, filteredExpanded, storySort },
      props,
    } = this;
    ...
    return (
      <Fragment>
        ...
        {roots.length || others.length ? (
          <Fragment>
            {roots.sort(storySort).map(({ id, name, children }) => (
              <Section key={id}>
