...
const getEnsured = location => {
  const { componentRoot, component } = getUrlData(location);
  return `${componentRoot}-${component}`;
};
...
const StoriesPanel = ({ stories }) => (
  <Location>
    {({ location }) => (
      <Explorer allowClick stories={stories} Link={LeafLink} ensure={[getEnsured(location)]} />
    )}
  </Location>
);
