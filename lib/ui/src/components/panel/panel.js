...
import { Tabs, Icons, IconButton, Separator } from '@storybook/components';
...
const AddonPanel = React.memo(({ panels, actions, selectedPanel, panelPosition }) => (
  <Tabs
    ...
    tools={
      <Fragment>
        <Separator force key="1" />
        <IconButton key="position" onClick={actions.togglePosition}>
          <Icons icon={panelPosition === 'bottom' ? 'bottombar' : 'sidebaralt'} />
        </IconButton>
        <IconButton key="visibility" onClick={actions.toggleVisibility}>
          <Icons icon="close" />
        </IconButton>
      </Fragment>
    }
