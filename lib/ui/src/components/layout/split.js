...
const splitStore = {
  get({ id, split }) {
    try {
      const rawValue = localStorage.getItem(`storybook-split-${id}-${split}`);
      return rawValue ? rawValue.split(',') : false;
    } catch (e) {
      console.error(e);
      return false;
    }
...
const MemSplit = styled(SplitObserver)(({ theme }) => ({
  ...
  '& > [data-type="Resizer"]': {
    minWidth: `${theme.layoutMargin}px!important`,
    minHeight: `${theme.layoutMargin}px!important`,
    borderLeft: ' 0 none',
    borderRight: ' 0 none',
    display: 'flex',
