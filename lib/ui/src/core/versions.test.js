...
const newResponse = makeResponse('4.0.0');
const oldResponse = makeResponse('2.0.0');
...
describe('versions API', () => {
  ...
  it('versionUpdateAvailable works for new versions', async () => {
    const store = createMockStore();
    const { api, init, state: initialState } = initVersions({ store });
    store.setState(initialState);
  ...
  it('versionUpdateAvailable works for old versions', async () => {
    const store = createMockStore();
    const { api, init, state: initialState } = initVersions({ store });
    store.setState(initialState);
