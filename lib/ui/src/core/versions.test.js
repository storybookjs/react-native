...
describe('versions API', () => {
  it('sets initial state with current version', async () => {
    ...
    expect(state.versions).toEqual({
      '3.0.0': { version: '3.0.0', current: true },
    });
  });
  ...
  it('sets initial state based on persisted versions', async () => {
    ...
    store.setState({
      versions: {
        '3.0.0': { info: '3-info', version: '3.0.0' },
        '4.0.0': { tag: 'latest', version: '4.0.0', info: '4-info' },
      },
    });
    const { state } = initVersions({ store });
    ...
    expect(state.currentVersion).toBe('3.0.0');
    expect(state.versions).toEqual({
      '3.0.0': { version: '3.0.0', current: true, info: '3-info' },
      '4.0.0': { version: '4.0.0', current: false, tag: 'latest', info: '4-info' },
    });
  });
  ...
  it('sets versions in the init function', async () => {
    ...
    expect(store.setState).toHaveBeenCalledWith(
      {
        versions: {
          '4.0.0': { version: '4.0.0', tag: 'latest', current: false },
          '3.0.0': { version: '3.0.0', current: true },
        },
  ...
  it('sets a new latest version if old version was cached', async () => {
    ...
    store.setState({
      versions: {
        '3.0.0': { version: '3.0.0' },
        '3.1.0': { tag: 'latest', version: '3.1.0' },
      },
    });
    ...
    expect(store.setState).toHaveBeenCalledWith(
      {
        versions: {
          '4.0.0': { version: '4.0.0', tag: 'latest', current: false },
          '3.1.0': { version: '3.1.0', current: false },
          '3.0.0': { version: '3.0.0', current: true },
        },
        lastVersionCheck: expect.any(Number),
      },
