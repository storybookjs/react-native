...
describe('stories API', () => {
  ...
  describe('selectStory', () => {
    ...
    it('does not navigate if you select a bogus story', () => {
      const navigate = jest.fn();
      const store = {
        getState: () => ({ viewMode: 'components', storiesHash }),
      };
      ...
      expect(() => selectStory('random')).toThrow(/does not exist/);
      expect(navigate).not.toHaveBeenCalled();
    });
    ...
    it('allows navigating to storyname, without kind (legacy api)', () => {
      const navigate = jest.fn();
      const store = {
        getState: () => ({ viewMode: 'components', componentId: 'a-1', storiesHash }),
      };
