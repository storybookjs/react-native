  ...
  async function init({ api: { versionUpdateAvailable, getLatestVersion, addNotification } }) {
    ...
    if (!lastVersionCheck || now - lastVersionCheck > checkInterval) {
      try {
        ...
        const newVersions = Object.values(versions).reduce(
          (acc, { tag, ...version }) => ({
            ...acc,
            [version.version]: {
              ...version,
              // Remove the tag field if it was equal to lates
              ...(tag !== 'latest' && { tag }),
            },
          }),
          {}
        );
        newVersions[latest.version] = {
          ...latest,
          tag: 'latest',
          current: currentVersion === latest.version,
        };
        await store.setState(
          { versions: newVersions, lastVersionCheck: now },
