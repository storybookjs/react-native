...
const PreviewConnected = React.memo(props => (
  <Consumer>
    {({ state, api }) =>
      api.renderPreview ? (
        api.renderPreview(
          state.storiesHash[state.storyId] ? state.storiesHash[state.storyId].kind : undefined,
          state.storiesHash[state.storyId] ? state.storiesHash[state.storyId].name : undefined
        )
      ) : (
        <Preview
          {...props}
