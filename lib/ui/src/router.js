...
import { Link, Location, Redirect } from '@reach/router';
...
const Route = ({ path, children }) => (
  <QueryMatch path={path}>{({ match }) => (match ? children : null)}</QueryMatch>
);

const RedirectLocation = ({ to, ...rest }) => <Redirect to={`?path=${to}`} {...rest} />;

export {
  QueryLink as Link,
  QueryMatch as Match,
  QueryLocation as Location,
  RedirectLocation as Redirect,
  Route,
};
