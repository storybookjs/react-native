...
describe('manager.ui.libs.hierarchy', () => {
  ...
  describe('createHierarchies', () => {
    test('should return empty array if stories are undefined', () => {
      const result = createHierarchies();

      expect(result).toHaveLength(0);
    });

    test('should return empty array if stories are empty', () => {
      const result = createHierarchies([]);
  ...
  describe('resolveStoryRoots', () => {
    test('should return empty root name when rootSeparator is undefined', () => {
      const [rootName, storyName] = resolveStoryRoots('some|name.item1');

      expect(rootName).toEqual('');
      expect(storyName).toEqual('some|name.item1');
    });

    test('should return array with separated root name and story name when rootSeparator is defined', () => {
      const [rootName, storyName] = resolveStoryRoots('some|name.item1', '\\|');
    ...
    test('should throw error when rootSeparator is found multiple times', () => {
      expect(() => {
        resolveStoryRoots('some|name|item1', '\\|');
      }).toThrow();
    });
  });
  ...
  describe('prepareStoriesForHierarchy', () => {
    ...
    test('should return filled root name when root separator is provided', () => {
      ...
      expect(result).toEqual([
        { kind: 'some|name.item1', namespaces: ['name', 'item1'], rootName: 'some' },
        { kind: 'another.space.20', namespaces: ['another', 'space', '20'], rootName: '' },
      ]);
    });
  });
