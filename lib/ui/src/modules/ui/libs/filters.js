...
import { resolveStoryHierarchy } from './hierarchy';
...
export function storyFilter(stories, filter, selectedKind, sortStoriesByKind, hierarchySeparator) {
  if (!stories) {
    return null;
  }
  ...
  const mapped = sorted.map(story => ({
    ...story,
    namespaces: resolveStoryHierarchy(story.kind, hierarchySeparator),
  }));
