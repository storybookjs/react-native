...
export const mapper = (state, props, { actions }) => {
  ...
  const { name, url, sortStoriesByKind, resolveStoryHierarchyNamespace } = uiOptions;
  const filteredStores = filters.storyFilter(stories, storyFilter, selectedKind, sortStoriesByKind);
  const storiesHierarchy = createHierarchy(filteredStores, resolveStoryHierarchyNamespace);
