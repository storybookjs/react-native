...
describe('manager.ui.components.left_panel.stories', () => {
  ...
  describe('events', () => {
    ...
    test('should call the onSelectStory prop when a story is clicked - hierarchySeparator is defined', () => {
      ...
      const wrap = mount(
        <Stories
          ...
          selectedStory="a2"
          selectedHierarchy={['another', 'space']}
          onSelectStory={onSelectStory}
        />
      );

      const a = () => wrap.find('[data-name="another"]').first();
      const b = () => wrap.find('[data-name="space"]').first();
      const c = () => wrap.find('[data-name="20"]').first();
      // const d = () => wrap.find('[data-name="b2"]').first();

      expect(a().length).toBeGreaterThan(0);
      // a().simulate('click', leftClick);

      expect(b().length).toBeGreaterThan(0);
      b().simulate('click', leftClick);

      expect(c().length).toBeGreaterThan(0);
      c().simulate('click', leftClick);
    ...
    test('should call the onSelectStory prop when a story is selected with enter key', () => {
      ...
      expect(b().length).toBeGreaterThan(0);
      b().simulate('keyDown', { keyCode: 13 });

      expect(c().length).toBeGreaterThan(0);
      c().simulate('keyDown', { keyCode: 13 });
