...
export default {
  ...
  selectInCurrentKind({ clientStore }, story) {
    clientStore.update(state => {
      const selectedStory = ensureStory(state.stories, state.selectedKind, story);
  ...
  setOptions(env, options) {
    ...
    clientStore.update(state => {
      const newOptions = pick(options, Object.keys(defaultState.uiOptions));
      const updatedUiOptions = {
        ...defaultState.uiOptions,
        ...newOptions,
      };
      const otherOptions = {};
