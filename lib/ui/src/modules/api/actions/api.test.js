...
describe('manager.api.actions.api', () => {
  ...
  describe('setOptions', () => {
    ...
    it('should update selectedDownPanel', () => {
      const clientStore = new MockClientStore();
      actions.setOptions(
        { clientStore, provider },
        { selectedDownPanel: 'storybooks/storybook-addon-knobs' }
      );

      const state = {
        uiOptions: {},
        selectedDownPanel: 'storybook/actions/actions-panel',
      };

      const stateUpdates = clientStore.updateCallback(state);
      expect(stateUpdates.selectedDownPanel).toEqual('storybooks/storybook-addon-knobs');
    });

    it('should keep current downPanel and output panel IDs', () => {
      const clientStore = new MockClientStore();
      actions.setOptions({ clientStore, provider }, { selectedDownPanel: null });
      ...
      const stateUpdates = clientStore.updateCallback(state);
      expect(stateUpdates.selectedDownPanel).toEqual('storybook/actions/actions-panel');
