...
const FrameWrap = styled.div(
  ...
  ({ filter }) =>
    filter
      ? {
          filter: filter === 'grayscale' ? 'grayscale(100%)' : `url('#${filter}')`,
        }
      : {}
);
  ...
  render() {
    ...
    return (
      <Fragment>
          ...
          <Toolbar
            ...
            left={[
              ...
              <IconButton
                key="filter"
                active={!!filter}
                onClick={() => this.setState({ filter: filter ? null : 'protanomaly' })}
              >
                {/* should be a dropdown with all sorts of options for every filter */}
                <Icons icon="mirror" />
              </IconButton>,
            ]}
            right={[
        ...
        <FrameWrap key="frame" offset={toolbarHeight} filter={filter}>
          <svg>
            <defs>
