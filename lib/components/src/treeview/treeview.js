...
import {
  Section as DefaultContainer,
  ListWrapper as DefaultList,
  DefaultLink,
  DefaultLeaf,
  DefaultHead,
  DefaultRootTitle,
...
const getLink = memoize(1)(LinkNode => LinkNode || DefaultLink);

const getHead = memoize(1)((HeadNode, LinkNode, prefix, events) =>
...
const getLeaf = memoize(1)((LeafNode, LinkNode, prefix, events) =>
...
const getFilter = memoize(1)(Filter => Filter || FilterField);
const getTitle = memoize(1)(TitleNode => TitleNode || DefaultRootTitle);
const getContainer = memoize(1)(SectionNode => SectionNode || DefaultContainer);
...
class TreeState extends PureComponent {
  ...
  render() {
    ...
    const Filter = getFilter(FilterNode);
    const List = getFilter(ListNode);
    const Branch = BranchNode || Tree;
    const Title = getTitle(TitleNode);
    const Link = getLink(LinkNode);
    const Leaf = getLeaf(LeafNode, Link, prefix, events);
    const Head = getHead(HeadNode, Link, prefix, events);
    const Section = getContainer(SectionNode);
...
TreeState.propTypes = {
  ...
  LeafNode: PropTypes.any,
  HeadNode: PropTypes.any,
  FilterNode: PropTypes.any,
  LinkNode: PropTypes.any,
  LeafNode: PropTypes.any,
  ListNode: PropTypes.any,
  HeadNode: PropTypes.any,
  TitleNode: PropTypes.any,
};
