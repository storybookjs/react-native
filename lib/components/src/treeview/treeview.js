...
const bubbleDown = ({ node, dataset }, e, { allowMultiple, allowNonLeaf }) => {
  const next = getNext({ path: node.path, dataset });
  const isLeaf = !dataset[next.path].children;

  const selected = getSelected({ dataset });
  const add = allowMultiple && isMacLike() ? e.metaKey : e.ctrlKey;
  const data = produce(dataset, set => {
    if (!(!allowNonLeaf && dataset[node.path].children && !isLeaf)) {
      // eslint-disable-next-line no-param-reassign
      set[next.path].isSelected = true;
    }
    // directory selected
    if (!isLeaf && next.children) {
      // eslint-disable-next-line no-param-reassign
      set[next.path].isExpanded = true;

      const { 0: nextNode } = next.children;

      // return bubbleDown({ node: dataset[nextNode], dataset }, e, { allowMultiple });
    }
    ...
    onKeyUp: (e, item) => {
      ...
      if (action === 'RIGHT') {
        ...
        this.setState(prevState => {
          const { dataset } = prevState;
          return { dataset: bubbleDown({ node: item, dataset }, e, this.props) };
        });
      }
      if (action === 'LEFT') {
        ...
        const replacement = produce(dataset, set => {
          ...
          if (!(!allowNonLeaf && dataset[item.path].children && !isLeaf)) {
            // eslint-disable-next-line no-param-reassign
            set[prev.path].isSelected = true;
          }
      ...
      if (action === 'UP') {
        ...
        const replacement = produce(dataset, set => {
          // eslint-disable-next-line no-param-reassign
          set[prev.path].isExpanded = true;
        });

        this.setState({ dataset: replacement });
      }
    },
