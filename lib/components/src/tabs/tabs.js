...
export const Tabs = ({ children, selectedPanel, onPanelSelect, absolute, bordered }) => {
  const list = React.Children.toArray(children).map(
    ({ props: { title, id, children: content } }, index) => ({
      active: selectedPanel ? id === selectedPanel : index === 0,
      title,
      id,
      render:
    ...
    <Wrapper absolute={absolute} bordered={bordered}>
      <TabBar role="tablist">
        {list.map(({ title, id, active }) => (
          <Tab key={`${id}-bar`} active={active} name={id} title={title} onSelect={onPanelSelect} />
        ))}
      </TabBar>
