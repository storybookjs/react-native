...
function applyPresets(presets, config, args = {}, extension) {
  let presetResult = new Promise(resolve => resolve(config));

  if (!presets.length) {
    return presetResult;
  }

  presets.forEach(({ preset, options }) => {
    ...
    if (extensionFn && typeof extensionFn === 'function') {
      ...
      presetResult.then(newConfig => {
        const result = extensionFn.call(preset, newConfig, combinedOptions);

        if (typeof result.then === 'function') {
          presetResult = result;
        } else {
          presetResult = new Promise(resolve => resolve(result));
        }
