 ...
 */
export const resolveAddonName = (name, { resolveFile }) => {
...
export const splitAddons = (addons, { resolveFile }) => {
  return addons.reduce(
    (acc, item) => {
      try {
        if (isObject(item)) {
          const { name } = resolveAddonName(item.name, { resolveFile });
          acc.presets.push({ ...item, name });
        } else {
          const { name, type } = resolveAddonName(item, { resolveFile });
          acc[type].push(name);
        }
      } catch (err) {
        logger.error(
