...
function outputStartupInformation(options) {
  ...
  const andPreview = previewTotalTime
    ? `and ${chalk.underline(prettyTime(previewTotalTime))} for preview`
    : '';
  ...
  console.log(
    boxen(
      stripIndents`
          ...
          ${chalk.underline(prettyTime(managerTotalTime))} for manager ${andPreview}`)}
...
export async function buildDevStandalone(options) {
  try {
    ...
    if (options.smokeTest) {
      ...
      if (!options.ignorePreview) process.exit(previewStats.toJson().warnings.length ? 1 : 0);
