...
export default configDir => {
  ...
  for (let i = 0; i < customConfigCandidates.length; i += 1) {
    const { path: configPath, ext: configExt } = customConfigCandidates[i];

    if (fs.existsSync(configPath)) {
      if (configExt !== '.js' && registerCompiler(interpret.extensions[configExt]) === 0) {
        logger.warn(`=> Custom configuration file ${configPath} is detected`);
        logger.warn(`   but impossible to import loader for ${configExt}`);
      }
      try {
