...
export default class ClientApi {
  ...
  storiesOf = (kind, m) => {
    ...
    api.add = (storyName, getStory, parameters) => {
      ...
      [this._globalParameters, localParameters, parameters].forEach(params => {
        if (params) {
          Object.keys(params).forEach(key => {
            ...
            } else if (isPlainObject(params[key]) && isPlainObject(allParam[key])) {
      ...
      // steal the context without actually rendering a story
      let c;
      const fakeStory = a => {
        c = a;
      };
      this._decorateStory(fakeStory, this._globalDecorators)({});

      const { hierarchyRootSeparator = '|', hierarchySeparator = /\/|\./ } = c;

      // lazily decorate the story when it's loaded
      const getDecorated = memoize(1)(() => this._decorateStory(getStory, decorators));
      this._storyStore.addStory(
        ...
        {
          rootSeparator: hierarchyRootSeparator,
          groupSeparator: hierarchySeparator,
        }
      );
      return api;
    };
