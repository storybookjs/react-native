...
const getContext = (() => {
  ...
  return decorateStory => {
    ...
    const configApi = new ConfigApi({ clearDecorators, storyStore, channel });
...
export default function start(render, { decorateStory } = {}) {
  ...
  const { clientApi, channel, configApi, storyStore } = getContext(decorateStory);
  ...
  const renderMain = forceRender => {
    ...
    // renderMain() gets executed after each action.
    // Actions will cause the whole story to re-render without this check.
