...
export default function start(render, { decorateStory } = {}) {
  ...
  if (isBrowser) {
    ...
    channel.on(Events.SET_CURRENT_STORY, ({ componentId }) => {
      if (!componentId) {
        throw new Error('should have componentId');
      }
      const data = storyStore.fromId(componentId);

      storyStore.setSelection(data);
      storyStore.setPath(componentId);
    });
