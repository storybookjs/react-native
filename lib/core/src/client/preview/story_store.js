...
export const splitPath = (path, { rootSeparator, groupSeperator }) => {
...
const removeFnValues = obj =>
  Object.entries(obj).reduce(
    (acc, [key, value]) =>
      typeof value === 'function' ? acc : Object.assign(acc, { [key]: value }),
    {}
  );
...
export default class StoryStore extends EventEmitter {
  ...
  fromPath = path => {
    const selectedId = path.split('/components/')[1] || '';

    try {
      const data = this._data[selectedId];
  ...
  extract() {
    // removes .fn from all stories
    return Object.entries(this._data).reduce(
      (a, [k, v]) => Object.assign(a, { [k]: removeFnValues(v) }),
      {}
    );
  }
  ...
  addStory({ kind, name, story, decorated, parameters = {} }, { rootSeparator, groupSeparator }) {
    ...
    h.reduceRight((childId, group, index, list) => {
      ...
      const depth = index;
      _data[path] = merge(existing, merge(group, { children: [childId], isRoot, depth, path }));
