...
const run = async () => {
  ...
  try {
    ...
    if (diffFiles.length === 0) {
      ...
      console.log(
        '💡 Run screenshot comparison without ignore regions to generate diff images first'
      );
      process.exit(0);
    }
    ...
    console.log(
      `npx @storybook/react-native-test screenshot-stories --ignore-regions "${regionsStr}"`
    );
    console.log(
      `npx @storybook/react-native-test compare-screenshots --ignore-regions "${regionsStr}"`
    );
