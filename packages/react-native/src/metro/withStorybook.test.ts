...
describe('withStorybook entry-point swapping', () => {
  const config = { resolver: {}, transformer: {} } as MetroConfig;
  let tmpDir: string;
  ...
  test('swaps entry point when STORYBOOK_ENABLED is true', () => {
    ...
    const { withStorybook: withSB } = require('./withStorybook');
...
describe('withStorybook websocket env overrides', () => {
  const config = { resolver: {}, transformer: {} } as MetroConfig;
